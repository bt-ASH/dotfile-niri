...
`NVIM_BUSTED_SHIM_NVIM`
   Default is `nvim`.  The Neovim binary to run, useful if you have multiple
   version of Neovim installed.

`NVIM_BUSTED_SHIM_CONFIG_HOME`
   Default is `test/xdg/config`.  Value used for the `$XDG_CONFIG_HOME`
   environment variable.

`NVIM_BUSTED_SHIM_DATA_HOME`
   Default is `test/xdg/local/share`.  Value used for the `$XDG_DATA_HOME`
   environment variable.

`NVIM_BUSTED_SHIM_STATE_HOME`
   Default is `test/xdg/local/state`.  Value used for the `$XDG_STATE_HOME`
   environment variable.

`NVIM_BUSTED_SHIM_CACHE_HOME`
   Default is `test/xdg/cache`.  Value used for the `$XDG_CACHE_HOME`
   environment variable.

Any environment variables will be passed through to the Neovim process.
