...
(
    ...
    if [ "$ENABLE_CLEANUP" = true ]; then
        ...
        cd "/home/ash/.cache/waypaper/Pi" && ls -t | tail -n +$DELETE_START | xargs -I {} rm -- {} 2>/dev/null
    fi
) &

bash ~/.config/niri/scripts/blur-overview.sh && matugen-update && (tmux source-file ~/.tmux.conf 2>/dev/null || true)
